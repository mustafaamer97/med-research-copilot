...
def render():
    ...
    st.info("Workflow validation is temporarily disabled during development.")

    # ==========================================
    # Research Question
    # ==========================================

    question_data = st.session_state.get("research_question", {})

    if question_data:

        st.subheader("Research Question")

        st.info(question_data.get("question", "No research question found."))

    # ==========================================
    # Search Query
    # ==========================================

    default_keywords = question_data.get("keywords", "")

    st.markdown("### 🔍 Search Strategy")

    query = st.text_area(
        "PubMed Search Query", value=default_keywords, height=120
    )
    ...
    with col1:
        ...
        number = st.selectbox(
            "Number of Papers", [5, 10, 20, 50], index=1
        )
    ...
    level4 = len([p for p in papers if p.get("evidence_level") == "Level 4"])
    ...
    c2.metric("Level 1", level1)

    c3.metric("Level 2", level2)

    c4.metric("Level 3", level3)

    c5.metric("Level 4", level4)
        ...
        with st.container():
            ...
            with btn1:
                ...
                if url:
                    ...
                    st.link_button("🔗 Open PubMed", url)

            with btn2:

                if st.button("💾 Save Paper", key=f"save_{idx}"):
